...
class JiraUtilsTests(unittest.TestCase):
    def test_YARN_10496(self):
        ...
        self.assertTrue(
            len(set_intersection) == len(expected_jira_ids),
            "Difference of expected jira ids / actual jira ids sets should be empty! "
            "Difference: " + str(missing_jira_ids),
        )

        expected_mappings = {
            "YARN-10624": "Support max queues limit configuration in new auto created queue, consistent with old auto created."
        }
